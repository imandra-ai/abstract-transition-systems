...
module Val = Smt_types_1.Value
module Var = Smt_types_1.Var
module T = Smt_types_1.Term
...
module Clause = struct
  ...
  let parse env : t P.t = P.(parsing "clause" (list (T.parse env) >|= Lit.Set.of_list))
end

module Def_constraint = struct
  (* result is always last *)
  type t =
    | Is_ite of T.t * T.t * T.t * T.t
    | Is_app of Var.t * T.t list * T.t
    | Is_eq of T.t * T.t * T.t

  let pp out = function
    | Is_ite (a,b,c,res) ->
      Fmt.fprintf out "(@[is-ite@ %a@ %a@ %a@ :res %a@])" T.pp a T.pp b T.pp c T.pp res
    | Is_app (f,l,res) ->
      Fmt.fprintf out "(@[is-app@ %a@ %a@ :res %a@])"
        Var.pp f (Fmt.Dump.list T.pp) l T.pp res
    | Is_eq (a,b,res) ->
      Fmt.fprintf out "(@[is-eq@ %a@ %a@ :res %a@])" T.pp a T.pp b T.pp res
end
...
module Trail = struct
  type kind =
    | Decision
    | BCP of Clause.t
    | Prop_def of { k: Def_constraint.t; subs: T.t list; }
...
module State = struct
  ...
  let resolve_conflict_ (self:t) : _ ATS.step option =
    ...
    | Conflict c when Clause.is_empty c ->
      ...
      Some (One (st', false, "learnt false"))
    | Conflict c ->
      begin match self.trail with
        | Trail.Nil ->
          Some (One (lazy (make (c::self.cs) self.trail Unsat), false, "empty trail")) (* unsat! *)
        | Trail.Cons {kind=Prop d;lit;next;_} when Clause.contains (-lit) c ->
          ...
          Some (One (lazy (make self.cs next (Conflict res)), false, expl))
        | Trail.Cons {kind=Prop _; next; _} ->
          Some (One (lazy (make self.cs next self.status), false, "consume"))
        | Trail.Cons {kind=Decision; _} ->
          (* start backjumping *)
          Some (One (lazy (make self.cs self.trail (Backjump c)), false, "backjump below conflict level"))
      end
    | _ -> None
  ...
  let backjump_ (self:t) : _ ATS.step option =
    ...
    | Backjump c when Clause.is_empty c ->
      Some (One (lazy (make (c::self.cs) self.trail Unsat), false, "learnt false"))
    | Backjump c ->
      let rec unwind_trail trail =
        ...
        | Trail.Cons {lit; _}
          ...
          Some (One (lazy (make (c :: self.cs) trail Searching), false, expl))
  ...
  let propagate self : _ ATS.step option =
    ...
    | Some (c,lit) ->
      ...
      Some (ATS.One (lazy (make self.cs trail Searching), false, expl))
    | None -> None
        ...
        |> Iter.flat_map_l
          (fun v ->
             let mk_ v =
               ...
               true, Fmt.sprintf "decide %a" Lit.pp v
             in
             [mk_ v; mk_ (-v)])
        |> Iter.to_rev_list
      in
